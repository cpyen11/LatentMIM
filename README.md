...
*[Simon Note]* The experiment found out that tyring to apply LatentMIM to extract semantic information from CSI-image (delay/angular-domain) failed. I originally hope to see that at least for k=2 unsupervised sementation, noise and paths can be successfully segmented, i.e. the embedding of patches contain paths and those contain noise only should be embeded into 2 clusters that are far from each other in the latent space. However, the CSI-image in delay/augular domain is sparse, i.e. only several patches contain significant paths where most pathces only contain noise. I conjecture that due the the MAE style masking strategy, it is hard to learn global semantics from the visible patches which most of do not include paths and this cause the failure.
...
<details> <summary>Challengs of Latent MIM</summary>
